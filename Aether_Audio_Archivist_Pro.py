...
def bootstrap_dependencies():
    ...
    deps = {"playwright": "playwright", "yt_dlp": "yt-dlp", "textual": "textual", "rich": "rich"}
...
class Archivist(Screen):
    ...
    def _download_yt(self, url, dest_path, artist, title, engine):
        out_base = str(dest_path.with_suffix(''))
            ...
            try:
                ...
                for query in queries:
                    ...
                    try:
                        ...
                        results = await loop.run_in_executor(self.executor, self._search_yt, query)
                    except:
                        continue
                ...
                # Download and tag in one go via thread pool
                loop = asyncio.get_running_loop()
                await loop.run_in_executor(
                    self.executor,
                    self._download_yt,
                    best['url'], dest, track['artist'], track['title'], self.engine
                )
