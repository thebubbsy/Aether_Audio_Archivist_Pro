...
# Post-bootstrap import to prevent crashes on fresh installs
import yt_dlp
            ...
            try:
                ...
                for query in queries:
                    if results:
                        break
                    try:
                        # Bolt Optimization: Use yt_dlp directly to avoid subprocess overhead
                        ydl_opts = {'quiet': True, 'extract_flat': True, 'noplaylist': True}
                        info = await asyncio.to_thread(functools.partial(yt_dlp.YoutubeDL(ydl_opts).extract_info, f"ytsearch5:{query}", download=False))
