...
class Archivist(Screen):
    ...
    async def _search_track_metadata(self, query: str):
        try:
            ...
            if proc.returncode != 0:
                self.log_kernel(f"SEARCH ERROR ({query}): {stderr.decode().strip()}")
                return []
            return [json.loads(line) for line in stdout.decode().strip().split("\n") if line]
        except (json.JSONDecodeError, OSError, ValueError) as e:
            self.log_kernel(f"SEARCH EXCEPTION ({query}): {e}")
            return []
        except Exception as e:
            # Re-raise unexpected exceptions like KeyboardInterrupt or SystemExit
            if isinstance(e, (KeyboardInterrupt, SystemExit, asyncio.CancelledError)):
                raise
            self.log_kernel(f"UNEXPECTED SEARCH ERROR ({query}): {e}")
            return []
        ...
        async with self.semaphore:
            ...
            try:
                ...
                for query in queries:
                    if results:
                        break
                    results = await self._search_track_metadata(query)
